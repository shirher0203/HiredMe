...
const staticQuestionRounds: QuestionRound[] = [
  {
    question: {
      id: "q1",
      question:
...
export const staticInterviewQuestions: InterviewQuestion[] =
  staticQuestionRounds.map((r) => r.question);

const evaluationByQuestionId: Map<string, AnswerEvaluation> = new Map(
  staticQuestionRounds.map((r) => [r.question.id, r.evaluation]),
);
